...
describe('Router', () => {
	...
	testRouter.routes = routeList;

	let originalPushState: (state, title: string, url?: string | null | undefined) => void;
	...
	test('일치하는 경로를 찾아야 합니다', () => {
		const route: Route = testRouter.findMatchingRoute('/');
		expect(route.name).toBe('home');
	});

	test('잘못된 경로에 대해 notfound 경로를 반환해야 합니다', () => {
		const route: Route = testRouter.findMatchingRoute('/invalid/path');
	...
	test('searchParams를 포함한 경로를 찾아야 합니다', () => {
		...
		const route: Route = testRouter.findMatchingRoute('/articles/1', queryParams);
